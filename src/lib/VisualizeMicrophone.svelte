<script lang="ts">
	...
	let levelRangeElement: HTMLInputElement;
	let logValue = "";
				...
				analyzer = Meyda.createMeydaAnalyzer({
					...
					callback: (features) => {
						...
						for (let i = 0; i < chroma.length; i++) {
							...
							// );
							ctx.beginPath();
							ctx.strokeStyle = `hsl(${(i / chroma.length) * 300}, 100%, ${50 * features.energy}%)`;
...
<input type="range" min="0" max="1" step="0.01" bind:this={levelRangeElement} />
